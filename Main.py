...
if __name__ == "__main__":
    ...
    print("\033[38;5;197m本程式目前正在開發中，部分功能仍不完全，僅供測試使用，敬請期待將來正式版本上線\033[0m\a\n")  # 輸出中文開發中提示，避免一般使用者誤用

    while True:
        print("這裡是「功能選擇平臺」，請選擇您要使用的功能")
        print("\033[38;5;43m0：顯示使用說明、1：開始計算分析、6：顯示開源許可（英文原版）、7：顯示開源許可（中文翻譯）、8：顯示作者宣告、9：結束程式運行")
        func = -1  # 「功能選擇平臺」的功能變數

        try:  # 讀取使用者輸入至功能變數，並嘗試轉換成整數
            func = Func.check_input("--> \033[0m", 0, 9)  # 呼叫 check_input() 函數讀取與檢查使用者輸入後存放至方案變數，依序傳入 詢問內容、最小數值、最大數值
