...
|-------------------|---------------------|-----------------|---------------|------------------|---------------------------------------------------------------------------------------|
| None              | 部分月票方案無法使用、計算與分析    | 邊緣 (Minor)      | 即將發布 (Fixed)  | 自行手動計算總搭乘金額      | 由於之前缺少資訊，所以無法計算以下月票<br>1. 北宜跨城際及雙北 $2,300<br>2. 澎湖縣公車 \$Unknown<br>3. 澎湖縣車船 \$Unknown |
> 如果您有發現任何其他這裡未列出的問題，歡迎向本存儲庫提出問題 (Issues) 與程式建議 (PR)  

## 未來功能 (Future Features)
| 未來版本         | 增加功能                                                | 開發狀態              | 優先順序       | 預定發布          |
|--------------|-----------------------------------------------------|-------------------|------------|---------------|
| ***1.0.10*** | 首次正式版本發佈                                            | 準備發布 (Completed)  | 高 (High)   | ***2025-08*** |
| ***1.0.10*** | 調整程式輸出文字內容與顯示效果                                     | 準備發布 (Completed)  | 高 (High)   | ***2025-08*** |
| ***1.0.10*** | 更新 `README.md`、`USER.md` 與 `DEVELOPER.md` 的內容       | 準備發布 (Completed)  | 中 (Medium) | ***2025-08*** |
| ***1.0.10*** | 新增一些驚喜的小彩蛋，供使用者發掘，提升用戶體驗                            | 準備發布 (Completed)  | 低 (Low)    | ***2025-08*** |
| ***1.0.10*** | 開放 北宜跨城際及雙北 \$2,300、澎湖縣公車 \$150、澎湖縣車船 \$400 月票方案的計算 | 準備發布 (Completed)  | 高 (High)   | ***2025-08*** |
| ***1.0.10*** | 大量新增與程式運行流程相關的提示訊息，方便得知目前的運作情形                      | 準備發布 (Completed)  | 中 (Medium) | ***2025-08*** |
| ***1.0.10*** | 修復在輸入票價金額與搭乘次數時，可能會因為出現錯誤導致無法正確儲存數據的問題              | 準備發布 (Completed)  | 高 (High)   | ***2025-08*** |
| ***1.0.10*** | 優化在輸出檔案 `TPASS_Result.txt` 中，時間戳記的顯示效果              | 準備發布 (Completed)  | 中 (Medium) | ***2025-08*** |
| ***1.0.10*** | 優化「臺北捷運+環狀線」的常客優惠票價顯示，現在顯示四捨五入後的整數值                 | 準備發布 (Completed)  | 中 (Medium) | ***2025-08*** |
| ***1.0.10*** | 現在會在總花費金額為 0 時出現額外的專屬提示內容，增加可讀性                     | 準備發布 (Completed)  | 中 (Medium) | ***2025-08*** |
| ***1.0.10*** | 現在會在輸出結果中包含 都會生活圈／縣市 的名稱，方便進行查找與判讀                  | 準備發布 (Completed)  | 中 (Medium) | ***2025-08*** |
| 1.0.11       | 增加對使用者輸入票價進行邊界檢查的功能，確保輸入票價的合理性                      | 功能規劃 (Planning)   | 低 (Low)    | 2025-??       |
| 1.1.0        | 增加可以寫入 `Diagnose.Log` 日誌檔案的功能                       | 功能規劃 (Planning)   | 中 (Medium) | 2025-??       |
| 1.1.1        | 增加更多例外情形處理                                          | 正在討論 (Discussing) | 低 (Low)    | 2025-??       |
| 1.1.2        | 提升跨系統顯示相容性，並開始發行 macOS/Linux 可執行檔                   | 功能規劃 (Planning)   | 低 (Low)    | 2026-??       |
| 1.2.0        | 優化調整程式邏輯與選單功能，為即將新增的 GUI 功能做準備                      | 功能規劃 (Planning)   | 低 (Low)    | 2026-??       |
